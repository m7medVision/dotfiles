...
QtObject {
  ...
  property string pluginsDir: home + "/.config/hyprshell/plugins"
  ...
  //   - the built-in bar option (`raw.bar`) is active when `bar.id` is
  //     missing or set to `raw.bar`.
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "raw.bar"
  ...
  function resolveEnabledId(id) {
    ...
    for (var candidate in installedPlugins) {
      ...
      var metadata = manifest && Util.isPlainObject(manifest.shellMeta) ? manifest.shellMeta : null
  ...
  // back, and so a plugin that is both a widget and a menu (raw.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "raw.workspaces", center: "raw.weather", right: "raw.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.shellMeta)
        ? candidateManifest.shellMeta : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "raw.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "raw.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `raw.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("raw.") === 0) {
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/hyprshell/plugins.
    var script = ""
