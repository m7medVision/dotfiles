...
Panel {
  id: root
  moduleName: "raw.monitor"
  ipcTarget: "raw.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function showBrightnessOsd(percent) {
    ...
    bar.shell.summon("raw.osd", JSON.stringify({
      icon: "brightness",
      value: percent
    }))
  }
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["display-text-size", String(px)]
  ...
  Process {
    ...
    // `brightness-display` races the hardware/driver and can
