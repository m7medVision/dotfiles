...
Panel {
  id: root
  moduleName: "raw.bluetooth"
  ipcTarget: "raw.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/bluetooth-device;
  ...
  function deviceCommand(action, address) {
    return ["bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via bluetooth-device.
  ...
  // the adapter came back on at the next boot. bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["bluetooth-power", adapter.enabled ? "off" : "on"])
  }

  IpcHandler {
    target: "raw.bluetooth"
