...
Panel {
  ...
  readonly property var mediaService: bar?.shell?.firstPartyServiceFor("raw.media")
  ...
  readonly property var candidateStreams: {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (String(n.name || "").indexOf("hyprshell_speaker_tuning") === 0) continue
      list.push(n)
    }
    return list
  }
  ...
  //
  // audio-output-sink resolves the *current* default output through any
