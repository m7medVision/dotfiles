...
Panel {
  id: root
  moduleName: "raw.network"
  ipcTarget: "raw.network"
  ...
  // The share card is its own panel plugin (raw.wifiqr) so a replacement
  ...
  function summonWifiQr(forceDetect) {
    ...
    bar.shell.summon("raw.wifiqr", JSON.stringify(payload))
  }
  ...
  // The speed test is its own panel plugin (raw.speedtest) so a
  ...
  function summonSpeedTest() {
    ...
    bar.shell.summon("raw.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "launch-floating-terminal-with-presentation"
