...
Item {
  ...
  // Fixed local path, no env var indirection.
  property string omarchyPath: Quickshell.env("HOME") + "/.local/share/hyprshell"
  property var shell: null
  property var manifest: null

  // Plugin lifecycle hooks. The host calls open(payloadJson) after
  // `shell-ipc-run shell summon raw.menu ...` and close() when hidden.
  ...
  property string defaultMenuPath: omarchyPath + "/default/hyprshell/hyprshell-menu.jsonc"
  property string userMenuPath: Quickshell.env("HOME") + "/.config/hyprshell/extensions/hyprshell-menu.jsonc"
  ...
  readonly property var providers: ({
    "fonts": {
      script: "current=$(font-current 2>/dev/null); font-list 2>/dev/null | while read -r f; do [[ -z $f ]] && continue; printf '%s\\t%s\\t%s\\n' \"$f\" \"$f\" \"$current\"; done",
      icon: "",
      volatile: true,
      actionFor: function(value) { return "font-set " + Util.shellQuote(value) }
    },
    "power-profiles": {
      script: "current=$(powerprofilesctl get 2>/dev/null); powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  // `shell-ipc-run shell summon omarchy.menu '{"menu":"system"}'`.
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "hyprshell-menu"
