...
Item {
  ...
  readonly property string currentBackgroundLink: stateHome + "/hyprshell/current/background"
  ...
  Process {
    id: bgSwitchProc
    command: ["bash", "-c", "background=$(theme-bg-switcher); [[ -n $background ]] && theme-bg-set \"$background\""]
    onExited: root.refreshBackground()
  }

  Process {
    id: themeSwitchProc
    command: ["bash", "-c", "theme=$(theme-switcher); [[ -n $theme ]] && theme-set \"$theme\" >/dev/null 2>&1 &"]
    onExited: root.refreshBackground()
  }
