...
Item {
  ...
  // The shell host injects omarchyPath (a fixed local path, no env var).
  ...
  property string omarchyConfigDir: home + "/.config/hyprshell"
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "raw.bluetooth").
  ...
  FileView {
    path: root.stateHome + "/hyprshell/current"
    watchChanges: true
    printErrors: false
  ...
  // and the flag is created/removed by hand.
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/hyprshell/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/hyprshell/toggles"
    watchChanges: true
    printErrors: false
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "hyprshell-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "hyprshell-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
