...
ShellRoot {
  ...
  // The shell host is the long-running entry point. Plugins live in sibling
  // directories under plugins/. This checkout is a fixed, fully-owned copy
  // (no upstream tracking, no external path env var needed).
  property string omarchyPath: home + "/.local/share/hyprshell"
  readonly property string shellPath: omarchyPath + "/shell"
  readonly property string firstPartyPluginsDir: shellPath + "/plugins"
  readonly property string defaultsPath: omarchyPath + "/config/hyprshell/shell.json"
  readonly property string userConfigPath: home + "/.config/hyprshell/shell.json"
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      position: "top",
      transparent: false,
      centerAnchor: "raw.clock",
      layout: {
        left: [{ id: "raw.workspaces" }],
        center: [{ id: "raw.clock", format: "dddd HH:mm" }],
        right: [{ id: "raw.audio" }]
      }
    },
    plugins: []
  ...
  readonly property string defaultBarId: "raw.bar"
  ...
  function isBarWidgetPanelPlugin(pluginId) {
    ...
    // panel loader (e.g. raw.menu); let that path handle them.
  ...
  function imagePickerItem() {
    var loader = panelLoaders["raw.image-picker"]
  ...
  IpcHandler {
    ...
    function open(imageDirs: string,
      ...
      return shell.summon("raw.image-picker", payload) ? "ok" : "unknown"
    }
